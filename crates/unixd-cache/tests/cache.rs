...
#[test]
fn credential_and_presentation_parts_are_refused() {
    for name in [
        "token",
        "access-token",
        "api_key",
        ...
        let refused = Key::builder("search").part(name, "value");
        assert!(
            matches!(refused, Err(Error::ForbiddenKeyPart(ref part)) if part == name),
            "{name}"
        );
    }
    for name in [
