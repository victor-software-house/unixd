...
| 2 | Run the work as its own Tokio task. | Run it in the leader's future. | A leader that disconnects would cancel the work for every waiter. A drain waits for the connections that wait on the work; work nobody waits for ends with the runtime. |
