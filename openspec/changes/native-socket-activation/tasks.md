...
- [ ] 2.1 Add a throwaway `examples/activation.rs` that adopts the stdin listener, answers one line per connection, and exits after an idle timeout; proof: `mise run verify` passes
- [ ] 2.2 Run it under launchd on macOS: first connection, idle exit, relaunch; proof: the three scenarios in `specs/activation` hold, with the timing recorded
