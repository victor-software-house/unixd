...
|--:|:--|:--|:--|
| 1 | `LinesCodec::new_with_max_length` over a reader limited to the cap, then `serde_json::from_str`. | A hand-written `take(max + 1).read_until` loop, as the consumer has. | The codec already stops at the cap and yields `None` after the error; the loop would make EOF, `\r`, and the exact-limit case our code. |
...
1. A frame sent without its newline before the client closes its write half
   is read as a frame. The client always sends the newline.
